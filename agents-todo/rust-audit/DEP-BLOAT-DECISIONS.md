...
writes `anyhow.workspace = true` by habit. Note `rustc-hash` is load-bearing in **`packages/lmao`**
...
| `xcut-dependency-bloat-sweep` F2 — DataFusion 47 is a 224-node graph behind an optional feature         | HIGH   | `a48f78a48`                                                                                                                                              | Feature and crate gone; `packages/lmao/Cargo.lock` is 144 rows / 45 darwin units. The `arrow` umbrella left with it.                                                                                                                                                                                                                                                                                  |
...
| `lmao-arrow` F8 — arrow default features pull chrono/chrono-tz                                          | MEDIUM | `1d28d7221 feat(lmao-rs): converge Arrow IPC on version 56`                                                                                              | `packages/lmao/Cargo.toml` has all four subcrates at `{ version = "56", default-features = false }`. `chrono-tz` is gone from the lockfile; `chrono` remains because `arrow-array` 56.2.1 lists it unconditionally.                                                                                                                                                                                   |
...
| 0   | baseline `packages/lmao`                                              | `RUSTC_WRAPPER= cargo build --workspace --all-targets`                                                   | exit 0, 15.57s                                                                                                                     |
