...
const WEBSITE = {
  ...
  vars: {
    SITE_URL: 'https://app.staging.example.com',
    ENVIRONMENT: 'staging',
    ISSUER: 'https://issuer.example.app',
  },
...
const PR7: DerivePullRequestConfigOptions = {
  stage: 'pr7',
  accountId: 'account-1',
...
describe('planPullRequestBindings for a flat config', () => {
  it('derives KV titles from the live staging titles and D1 names from the staging names', () => {
    const plan = planPullRequestBindings(CONFIG, 'pr7', LIVE_KV);
  ...
  it('refuses a staging KV namespace whose live title has no staging segment', () => {
    expect(() => planPullRequestBindings(CONFIG, 'pr7', [{ id: 'kv-staging', title: 'shared' }])).toThrow(
  ...
  it('refuses a staging D1 database whose name has no staging segment', () => {
    ...
    expect(() => planPullRequestBindings(config, 'pr7', LIVE_KV)).toThrow(/no exact staging segment/);
  });
  ...
  it('refuses an all-pinned template, so the caller creates no resources for a stage it cannot route', () => {
    ...
    expect(() => planPullRequestBindings(config, 'pr7', LIVE_KV)).toThrow(/pinned/);
    expect(() => planPullRequestBindings(config, 'pr7', LIVE_KV)).toThrow(/next\.example\.com/);
  });
  ...
  it('refuses a template whose name does not end in -staging, since it is not the staging build', () => {
    ...
    expect(() => planPullRequestBindings(config, 'pr7', LIVE_KV)).toThrow(/-staging/);
  });
});

describe('derivePullRequestStageConfig for a flat config', () => {
  const derived = derivePullRequestStageConfig(CONFIG, PR7);
  ...
  it('refuses a template whose name does not end in -staging, since it is not the staging build', () => {
    expect(() => derivePullRequestStageConfig(configWith({ name: 'site-preview' }), PR7)).toThrow(/-staging/);
  });

  it('leaves an explicitly routeless template routeless', () => {
    expect(derivePullRequestStageConfig(configWith({ routes: [] }), PR7).routes).toEqual([]);
  });

  it('derives vars by hostname label and ENVIRONMENT verbatim', () => {
    expect(derived.vars).toEqual({
      SITE_URL: 'https://app.pr7.example.com',
      ENVIRONMENT: 'pr7',
      ISSUER: 'https://issuer.example.app',
    });
  });
  ...
  it('does not add an ENVIRONMENT var the template does not declare', () => {
    const config = configWith({ vars: { ISSUER: 'https://issuer.example.app' } });
    expect(derivePullRequestStageConfig(config, PR7).vars).toEqual({ ISSUER: 'https://issuer.example.app' });
  });
  ...
  it('leaves an unlabelled service binding shared', () => {
    const config = configWith({ services: [{ binding: 'SHARED', service: 'auth-service' }] });
    expect(derivePullRequestStageConfig(config, PR7).services).toEqual([
  ...
  it('materialises no section a minimal template does not declare', () => {
    expect(derivePullRequestStageConfig(parseFlatWranglerConfig('{"name":"x-staging"}'), PR7)).toEqual({
      name: 'x-pr7',
    });
  });

  it('refuses a KV binding with no derived counterpart', () => {
    expect(() => derivePullRequestStageConfig(CONFIG, { ...PR7, kvNamespaceIds: new Map() })).toThrow(
      /KV namespace id/,
    );
  });

  it('refuses a D1 binding with no derived counterpart', () => {
    expect(() => derivePullRequestStageConfig(CONFIG, { ...PR7, d1DatabaseIds: new Map() })).toThrow(/D1 database id/);
  });

  it('refuses an R2 bucket with no staging segment, which the stage would otherwise share', () => {
    const config = configWith({ r2_buckets: [{ binding: 'MEDIA', bucket_name: 'shared-media' }] });
    expect(() => derivePullRequestStageConfig(config, PR7)).toThrow(/no exact staging segment/);
  });
});
