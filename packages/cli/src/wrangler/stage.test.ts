...
describe('Wrangler deployment stage convention', () => {
  ...
  it('derives the app staging block without changing inherited/static semantics', () => {
    ...
    expect(derived).toContain('pattern = "pr123.example.test"');
    expect(derived.slice(derived.indexOf('[env.pr123]'))).not.toContain('next.example.test');
  ...
  it('refuses an all-pinned TOML template before any resource is named', () => {
...
`;
  ...
  it('derives backend resources from staging while preserving provider and DO identities', () => {
    ...
    expect(derived).toContain(`namespace_id = "${signupId}"`);
    expect(derived).toContain(`namespace_id = "${loginId}"`);
    expect(derived).toContain('id = "kv-alias-pr123-id"');
    expect(derived).toContain('id = "kv-org-pr123-id"');
    expect(derived).toContain('database_name = "app-pr123-db"');
    expect(derived).toContain('database_id = "d1-pr123-id"');
    expect(derived).toContain('bucket_name = "app-media-pr123"');
    expect(derived).toContain('service = "app-backend-pr123"');
    expect(derived).toContain('service = "auth-service"');
    expect(derived).toContain('ENVIRONMENT = "pr123"');
    expect(derived).toContain('TLD_DOMAIN = "pr123.example.test"');
    expect(derived).toContain('BUILD_ID = "staging-20260716-2"');
    expect(derived).toContain('allowed_sender_addresses = ["login@mail.pr123.example.test"]');
    expect(derived).toContain('MAIL_RECIPIENTS = "login-test@pr123.example.test,invite-test@pr123.example.test"');
    expect(derived).toContain('APP_ORIGIN = "https://app.pr123.example.test"');
    expect(derived).toContain('class_name = "QueueDO"');
    expect(derived).toContain('new_sqlite_classes = ["QueueDO", "StoreDO"]');
    expect(derived).not.toContain('pr456');
  });
