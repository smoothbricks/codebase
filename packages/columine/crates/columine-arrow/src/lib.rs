...
//!
//! Ports, unified from two drifted artifact variants (this crate is the
//! base per the port dossier; the consumer delta was audited hunk-by-hunk):
//!
//! - `parsing/columns.zig` — [`columns`]: `EventColumns` (base 4-column),
//!   `ColumnStorage`/`DynamicColumns` (N-column extraction), and the
//!   transactional `VariableValueReservation`. The consumer artifact is this
//!   file minus the whole `EventColumns` base path (+ comment
//!   rewording); nothing else drifted.
//! - `arrow/dynamic_schema.zig` — [`schema`]: TS-generated schema bytes +
//!   `SignalSchemaField` metadata. Consumer delta: deletion of
...
//!   optimization was forward-ported to this crate before this port).
//! - `arrow/ipc_writer.zig` — [`ipc`]: consumer delta is deletion of the base
...
//!   relocation in this crate's base path is un-forward-ported drift, not
//!   semantics.
//!
