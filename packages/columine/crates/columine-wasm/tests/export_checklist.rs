//! Pins the wasm export surface against the columine VM ABI (the Zig `columine.wasm` ground
//! truth: all 81 function exports (enumerated from the Zig artifact's export
//! section) plus the exported memory... except memory is IMPORTED here
//! (`--import-memory`, parity with build.zig), so only functions are listed.
//!
//! The artifact-parsing test is `#[ignore]`d because it needs the built
//! artifact; `just wasm` builds and then runs it with `--include-ignored`.

/// Every function export of the Zig reducer_vm.wasm (its export section,
/// parsed mechanically — see specs/axe/91-zig-to-rust-port.md stage 5).
pub const ZIG_EXPORTS: [&str; 56] = [
...
fn built_wasm_exports_every_zig_symbol() {
    ...
    let fn_names: std::collections::HashSet<&str> = exports
        ...
        .map(|(n, _)| n.as_str())
        .collect();
    let missing: Vec<&&str> = ZIG_EXPORTS
        .iter()
        .filter(|n| !fn_names.contains(**n))
        .collect();
    assert!(
        missing.is_empty(),
        "exports missing vs Zig reducer_vm.wasm: {missing:?}"
    );
}
