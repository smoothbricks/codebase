...
const NEW: CommandSpec = CommandSpec {
    ...
    about: &[
        ...
        "A build slot is a stable mount path, `mnt/<owner>/<repo>/slot@<n>`, held by one workspace at a time and released when that workspace is removed or renamed, so the next tenant of slot n builds through byte-identical absolute paths. Path identity is no longer what makes the compiler cache hit: cargo computes `-C metadata` path-independently for workspace members from 1.97, and the bundled sccache keys cwd, the blanket CARGO_* values and the argument bytes against the request cwd, so every workspace shares one cache at any mount. A slot is for what neither reaches — a crate whose output records an `env!(\"CARGO_MANIFEST_DIR\")` env-dep fails closed across paths, tooling that persists absolute paths across tenant generations keeps working, and an unpatched sccache or an older cargo still needs the path itself. Main cannot take a slot: its mount is fixed by the checkout layout.",
    ],
    options: &[
        Opt {
            spelling: "--ref <rev>",
