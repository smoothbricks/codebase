...
const NEW: CommandSpec = CommandSpec {
    ...
    about: &[
        ...
        "A build slot is a stable mount path, `mnt/<owner>/<repo>/slot@<n>`, held by one workspace at a time and released when that workspace is removed or renamed, so the next tenant of slot n builds through byte-identical absolute paths. That path identity is the whole feature: cargo derives `-C metadata` from a package id carrying the absolute manifest directory, and sccache hashes the compiler's physical working directory, so the same sources built at two paths are two different compilations sharing no compile cache. Every workspace already routes rustc through sccache; a slot is what makes cargo's own `-C metadata` line up too. Main cannot take a slot: its mount is fixed by the checkout layout.",
    ],
    options: &[
        Opt {
            spelling: "--ref <rev>",
