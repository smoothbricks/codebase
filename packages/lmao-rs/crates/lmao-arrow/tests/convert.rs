...
fn real_root(trace: &str, span_id: u32, logs: usize) -> SpanBuffer {
    ...
    let mut buf = SpanBuffer::start(identity, 8, &anchor, &clock);
    for _ in 0..logs {
...
fn converts_core_span_buffers_with_overflow() {
    ...
    // Root 1: 2 fixed rows + 20 logs; root 2: 2 + 3.
    assert_eq!(batch.num_rows(), 22 + 5);
...
fn pyarrow_reads_our_ipc() {
    ...
    assert_eq!(
        lines.next().unwrap(),
        "timestamp,trace_id,thread_id,span_id,parent_thread_id,parent_span_id,entry_type,message"
    );
}
