...
| `lmao-vs-wasm.bench.ts`           | consumer runtime test suite                                             | real lmao vs WASM-backed SpanBuffer (spec 01q)                                         | needs `zig build` artifacts — blocked                                                                                                                                                                                                                                                                                         |
...
Environment notes: zig 0.16.0 is available via the smoothbricks devenv
(`cd tooling/direnv && devenv shell -q -- zig ...`) and was used to rebuild `allocator.wasm`. Rust→WASM remains
uncompiled here (nix rustc has no wasm32 std, no rustup, devenv has no rust) — approach (b) is argued from measured
boundary costs + the confirmed JS-vs-WASM numbers rather than a compiled Rust-WASM artifact.
