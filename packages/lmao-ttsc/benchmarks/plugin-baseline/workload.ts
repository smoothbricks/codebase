...
const childCoverageOp = defineOp('plugin-baseline-child-coverage', async (ctx) => {
  for (let index = 0; index < 12; index++) {
    ctx.log.info('Processing complete').index(200 + index).detail('overflow-coverage');
  }
...
async function semanticSnapshot() {
  ...
  if (!result.success || result.value.processed !== ROOT_LOG_COUNT || !childResult.success || !childResult.value.child) {
