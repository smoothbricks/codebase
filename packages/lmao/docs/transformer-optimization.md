...
- **Profile-guided emission (novel loop).** Test suites already run under `SQLiteTracer` (`.cache/trace-results.db`).
  The plugin can read that database as build input: measured column-usage frequencies and span row-count distributions
