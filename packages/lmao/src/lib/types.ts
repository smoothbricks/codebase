 ...
 * **Type Parameter**
 *
 * `T` is the tag attribute schema. When provided, SpanBuffer gets typed setters
 * for all schema fields (e.g., `buffer.userId(pos, val)`).
 * Defaults to `TagAttributeSchema` for backwards compatibility.
 *
 * **Naming Convention**
 *
 ...
 */
/** Empty schema - no user-defined columns. Used as default type parameter. */
type EmptySchema = Record<string, never>;

export type SpanBuffer<T extends TagAttributeSchema = EmptySchema> = TypedColumnBuffer<T> & {
  // ============================================================================
