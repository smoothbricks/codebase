...
function generateConstructorCode(columnMeta: ColumnMeta[]): string {
  const lines: string[] = [
    ...
    "  // Allocate a scratch buffer to hold trace_id bytes (can't use bump ptr directly as allocIdentityRoot uses it)",
    '  const scratchPtr = opts.allocator.alloc8B();',
    '  opts.allocator.u8.set(traceIdBytes, scratchPtr);',
    '  this._identityPtr = opts.allocator.allocIdentityRoot(scratchPtr, traceIdBytes.length);',
    '  opts.allocator.free8B(scratchPtr);',
    '} else {',
