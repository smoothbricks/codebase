...
const usdBasis = (value: bigint) => Basis<'USD'>(value);
const jpyBasis = (value: bigint) => Basis<'JPY'>(value);
const btcBasis = (value: bigint) => Basis<'BTC'>(value);

describe('basisAdd', () => {
  it('adds two basis values of same currency', () => {
    const result = basisAdd(Basis<'USD'>(1_000_000_000n), Basis<'USD'>(2_000_000_000n));
    expect(result).toBe(usdBasis(3_000_000_000n));
  });
});
...
describe('basisSubtract', () => {
  it('subtracts two basis values of same currency', () => {
    ...
    expect(result).toBe(usdBasis(2_000_000_000n));
  });
...
describe('basisMultiply', () => {
  it('multiplies basis by scalar', () => {
    ...
    expect(result).toBe(usdBasis(5_000_000_000n));
  });
});
...
describe('toBasis', () => {
  it('converts USD Amount to Basis (scale up by 10^(10-2) = 10^8)', () => {
    ...
    expect(result).toBe(usdBasis(10_000_000_000n));
  });
  ...
  it('converts JPY Amount to Basis (scale up by 10^(8-0) = 10^8)', () => {
    ...
    expect(result).toBe(jpyBasis(100_000_000n));
  });
  ...
  it('handles large amounts', () => {
    ...
    expect(result).toBe(usdBasis(10_000_000_000_000_000n));
  });
});
