...
📖 Setup guide: https://github.com/smoothbricks/smoothbricks/blob/main/packages/patchnote/docs/GETTING-STARTED.md
...
Workflow functions accept `Partial<PatchnoteConfig>` instead of full config:

```typescript
export async function createUpdateCommit(
  config: Partial<PatchnoteConfig>, // Not full PatchnoteConfig
...
- **Auth detection:** If `vars.PATCHNOTE_APP_ID` is set → GitHub App mode; otherwise → PAT mode
...
GH_TOKEN: ${{ steps.app-token.outputs.token || secrets.PATCHNOTE_TOKEN }}
```
...
- **Conditional AI** - `vars.PATCHNOTE_SKIP_AI` variable can disable AI at runtime
...
patchnote generate-workflow --enable-ai

# Custom schedule
patchnote generate-workflow --schedule "0 3 * * 1"
```
...
gh secret set PATCHNOTE_TOKEN --org YOUR_ORG

# To use GitHub App (takes priority if both configured):
# Add variable and secret
gh variable set PATCHNOTE_APP_ID --org YOUR_ORG
gh secret set PATCHNOTE_APP_PRIVATE_KEY --org YOUR_ORG

# To disable AI at runtime: Add variable
gh variable set PATCHNOTE_SKIP_AI --body "true" --org YOUR_ORG
```
...
5. Run CLI with `GH_TOKEN: ${{ secrets.PATCHNOTE_TOKEN }}`

**Workflow Steps (GitHub App):**

1. Checkout repo with full history (`fetch-depth: 0`)
2. Generate GitHub App token using `actions/create-github-app-token@v2`
   - Reads `PATCHNOTE_APP_ID` from repository variables
   - Reads `PATCHNOTE_APP_PRIVATE_KEY` from repository secrets
...
3. Add organization variables and secrets:
   - Variable: `PATCHNOTE_APP_ID` (App ID from GitHub App settings)
   - Secret: `PATCHNOTE_APP_PRIVATE_KEY` (Private key PEM file content)
...
- Generates `.patchnote.json` or `.patchnote.ts` config file
...
1. Set up authentication (choose one):
   - Option A: PAT (5 min) - Add PATCHNOTE_TOKEN secret
   - Option B: GitHub App (15 min) - Add PATCHNOTE_APP_ID variable + private key secret
...
```bash
nx run @smoothbricks/patchnote:update-deps -- [CLI_FLAGS]
```
...
nx run @smoothbricks/patchnote:update-deps -- --verbose --skip-ai

# All CLI flags work with -- separator
nx run @smoothbricks/patchnote:update-deps -- --dry-run
```
...
nx run @smoothbricks/patchnote:lint  # Run linting
...
- `tooling/patchnote.ts` (priority 1, supports custom logic and hooks)
- `tooling/patchnote.json` (priority 2, simple configuration)
