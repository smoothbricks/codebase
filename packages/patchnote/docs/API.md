...
This document provides a comprehensive reference for using `patchnote` programmatically in your own scripts and tools.
...
# or
npm install @smoothbricks/patchnote
```
...
```typescript
async function loadConfig(): Promise<PatchnoteConfig>;
```
...
- Priority: `tooling/patchnote.ts` > `tooling/patchnote.json`
...
```typescript
function mergeConfig(partialConfig: Partial<PatchnoteConfig>): PatchnoteConfig;
```
...
```typescript
function defineConfig(config: Partial<PatchnoteConfig>): Partial<PatchnoteConfig>;
```
...
```typescript
async function updateDeps(config: PatchnoteConfig, options: UpdateDepsOptions): Promise<void>;
```
...
```typescript
async function updateExpo(config: PatchnoteConfig, options: UpdateExpoOptions): Promise<void>;
```
...
```typescript
async function generateWorkflow(config: PatchnoteConfig, options: GenerateWorkflowOptions): Promise<void>;
```
...
```typescript
import { createConsoleLogger } from '@smoothbricks/patchnote';
...
```typescript
import { updateDeps, mergeConfig } from '@smoothbricks/patchnote';
...
```typescript
import { updateExpo, mergeConfig } from '@smoothbricks/patchnote';
...
```typescript
import { generateWorkflow, loadConfig } from '@smoothbricks/patchnote';
...
```typescript
import { updateDeps, loadConfig } from '@smoothbricks/patchnote';
