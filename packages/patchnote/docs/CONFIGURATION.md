...
This document provides a comprehensive reference for all `patchnote` configuration options.
...
1. **`tooling/patchnote.ts`** - TypeScript config with type safety (priority 1)
2. **`tooling/patchnote.json`** - JSON config for simple setups (priority 2)
...
1. **Generate initial config**: Run `patchnote generate-syncpack --expo-sdk 52` to create `.syncpackrc.json` with Expo
   rules
...
3. **Enable preservation**: Set `preserveCustomRules: true` in your patchnote config
...
This keeps a clean separation: patchnote manages Expo SDK rules, you manage project-specific rules in the syncpack
