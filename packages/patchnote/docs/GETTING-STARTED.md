# Getting Started with patchnote
...
- Nx monorepo with patchnote installed
...
2. Give it a name: "patchnote for [ORG_NAME]"
...
- Set repository variable `PATCHNOTE_SKIP_AI=true`, or
...
2. Click **New organization secret**
3. Configure:
   - **Name**: `PATCHNOTE_APP_PRIVATE_KEY`
...
In each repository where you want to run patchnote:
...
- Set repository variable `PATCHNOTE_SKIP_AI=true`

#### 4.2 Commit and Push

```bash
git add .github/workflows/update-deps.yml
git commit -m "chore: add patchnote workflow"
git push
```
...
For repositories using [devenv](https://devenv.sh) for development environments, patchnote automatically handles Nix
tooling.
...
When Nix is enabled, patchnote can update:
...
For accurate package version detection, patchnote uses [dix](https://github.com/faukah/dix) to compare Nix derivation
...
Create `tooling/patchnote.json` to customize behavior:
...
- PAT: Verify `PATCHNOTE_TOKEN` secret exists and has correct scope
...
2. Add the organization variable: `PATCHNOTE_APP_ID`
3. Add the organization secret: `PATCHNOTE_APP_PRIVATE_KEY`
4. **That's it!** The workflow will automatically use GitHub App on next run

The workflow detects auth method at runtime:

- If `PATCHNOTE_APP_ID` is set → GitHub App (priority)
- Otherwise → PAT fallback

**Optional cleanup:** Delete old `PATCHNOTE_TOKEN` secret after confirming GitHub App works.
...
1. Run validation: `patchnote validate-setup` (GitHub App only)
...
Congratulations! Your patchnote is now set up and ready to keep your dependencies up to date automatically.
