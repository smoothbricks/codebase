...
This guide covers common issues and solutions for `patchnote`. Issues are organized by category for easy navigation.
...
4. **Verify workflow configuration:**
   ```yaml
   env:
     GH_TOKEN: ${{ secrets.PATCHNOTE_TOKEN }}
   ```
...
1. **Verify App ID:**
   ...
   - Compare with `PATCHNOTE_APP_ID` variable value
...
2. **Check secret/variable access:**
   - Go to Organization Settings → Secrets and variables → Actions
   - Click on `PATCHNOTE_APP_ID` (Variables tab)
   - Click on `PATCHNOTE_APP_PRIVATE_KEY` (Secrets tab)
   - Verify "Repository access" includes your repository
...
**Problem:** PRs created by patchnote don't trigger other CI workflows.
...
```
Error: Failed to parse config file at tooling/patchnote.json
...
3. **Use TypeScript config instead:**
   - Rename to `tooling/patchnote.ts`
...
4. **Open an issue:**
   ...
   - Include:
     ...
     - Output of `patchnote validate-setup`
