 ...
 */
export async function generateSyncpack(config: PatchnoteConfig, options: GenerateSyncpackOptions): Promise<void> {
  ...
  if (!sdkVersion) {
    if (!config.expo?.enabled) {
      ...
      config.logger?.error('Usage: patchnote generate-syncpack --expo-sdk 52');
      return;
    }
