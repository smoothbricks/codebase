/**
 * Initialize patchnote in a project
 */
...
function generateJSONConfig(options: {
  ...
  const config: Partial<PatchnoteConfig> = {
    expo: {
...
function generateTypeScriptConfig(options: {
  ...
  return `import { defineConfig } from '@smoothbricks/patchnote';
 ...
 * Initialize patchnote in a project
 */
export async function init(config: PatchnoteConfig, options: InitOptions): Promise<void> {
  const repoRoot = config.repoRoot || (await getRepoRoot());

  p.intro('🚀 Initializing patchnote');

  // Check if already initialized
  const toolingDir = safeResolve(repoRoot, 'tooling');
  const configJSONPath = safeResolve(toolingDir, 'patchnote.json');
  const configTSPath = safeResolve(toolingDir, 'patchnote.ts');
  ...
  if (!options.yes) {
    ...
    p.note(
      'The workflow supports two authentication methods (auto-detected at runtime):\n\n' +
        ...
        '    • Add secret: PATCHNOTE_TOKEN\n\n' +
        '  Option B: GitHub App (15 minutes, recommended)\n' +
        '    • Add variable: PATCHNOTE_APP_ID\n' +
        '    • Add secret: PATCHNOTE_APP_PRIVATE_KEY\n\n' +
        'You can switch methods anytime without regenerating the workflow.\n\n' +
        '📖 Full guide: https://github.com/smoothbricks/smoothbricks/blob/main/packages/patchnote/docs/GETTING-STARTED.md',
    ...
    const formatPrompt = await p.select({
      ...
      options: [
        { value: 'json', label: 'JSON (tooling/patchnote.json)', hint: 'Simple configuration' },
        {
          value: 'ts',
          label: 'TypeScript (tooling/patchnote.ts)',
  ...
  const configFileName = useTypeScript ? 'patchnote.ts' : 'patchnote.json';
  ...
  if (generateWorkflowFile) {
    const workflowConfig: PatchnoteConfig = {
      ...config,
      repoRoot,
    };
  ...
  nextSteps += '     • Add secret: gh secret set PATCHNOTE_TOKEN --org YOUR_ORG\n\n';
  nextSteps += '   Option B: GitHub App (15 min, recommended)\n';
  nextSteps += '     • Create app and add: PATCHNOTE_APP_ID variable\n';
  nextSteps += '     • Add secret: PATCHNOTE_APP_PRIVATE_KEY\n\n';
  stepNumber++;
