...
/**
 * Validate complete patchnote setup
...
export async function validateSetup(
  ...
  if (hasFailures) {
    ...
    logger.info(
      '\n📖 See setup guide: https://github.com/smoothbricks/smoothbricks/blob/main/packages/patchnote/docs/SETUP.md',
    );
    return 1;
  }

  logger.info('✅ All checks passed! Your patchnote setup is ready.');
  logger.info('\n🚀 Next steps:');
 ...
 * This validates that PATCHNOTE_APP_ID and PATCHNOTE_APP_PRIVATE_KEY are configured
 */
async function checkCanGenerateToken(
  ...
  const appId = process.env.PATCHNOTE_APP_ID;
  const privateKey = process.env.PATCHNOTE_APP_PRIVATE_KEY;

  if (!appId || !privateKey) {
    return {
      success: false,
      message: 'GitHub App credentials not configured',
      details: 'Set PATCHNOTE_APP_ID and PATCHNOTE_APP_PRIVATE_KEY environment variables or repository secrets',
    };
  }
...
async function checkConfig(repoRoot?: string): Promise<ValidationResult> {
  try {
    ...
    return {
      ...
      details: 'Run: patchnote init (to create config)',
    };
  }
}
