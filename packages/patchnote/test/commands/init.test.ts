...
describe('init command', () => {
  const _defaultConfig: PatchnoteConfig = {
    repoRoot: '/test-repo',
    prStrategy: {
  ...
  describe('Config generation - TypeScript format', () => {
    test('should generate TypeScript config structure', () => {
      ...
      const tsConfig = `import { defineConfig } from '@smoothbricks/patchnote';
...
`;
  ...
  describe('Unified auth (runtime detection)', () => {
    test('should not require auth type selection', () => {
      ...
      // - If PATCHNOTE_APP_ID is set → GitHub App
  ...
  describe('File path generation', () => {
    test('should generate correct JSON config path', () => {
      ...
      const configPath = `${repoRoot}/tooling/patchnote.json`;

      expect(configPath).toBe('/test-repo/tooling/patchnote.json');
    });

    test('should generate correct TypeScript config path', () => {
      const repoRoot = '/test-repo';
      const configPath = `${repoRoot}/tooling/patchnote.ts`;

      expect(configPath).toBe('/test-repo/tooling/patchnote.ts');
    });
  ...
  describe('Overwrite logic', () => {
    test('should detect existing config', () => {
      const existingConfigPaths = ['/test-repo/tooling/patchnote.ts', '/test-repo/tooling/patchnote.json'];

      // Simulate finding existing config
      const existingConfig = existingConfigPaths[0]; // TS config exists

      expect(existingConfig).toBe('/test-repo/tooling/patchnote.ts');
    });

    test('should allow overwrite with --yes flag', () => {
      const _existingConfig = '/test-repo/tooling/patchnote.json';
      const options: InitOptions = {
        yes: true,
        dryRun: false,
      };
