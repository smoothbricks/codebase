...
describe('mergeConfig', () => {
  ...
  test('should merge partial config with explicit projects', () => {
    ...
    expect(merged.prStrategy.stackingEnabled).toBe(true); // from defaults
    expect(merged.ai.provider).toBe('zai'); // from defaults (free tier)
  });
...
describe('defaultConfig', () => {
  test('should have sensible defaults', () => {
    ...
    expect(defaultConfig.ai.provider).toBe('zai'); // Free tier by default
...
describe('loadConfig', () => {
  ...
  test('should load and merge config from tooling/patchnote.json', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'patchnote-test-'));
    ...
    writeFileSync(join(toolingDir, 'patchnote.json'), JSON.stringify(userConfig, null, 2));
    ...
    expect(config.ai.provider).toBe('zai'); // Free tier by default
  });

  test('should load config from tooling/patchnote.json', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'patchnote-test-'));
    const toolingDir = join(tmpDir, 'tooling');
    mkdirSync(toolingDir, { recursive: true });
  ...
  test('should prefer tooling/patchnote.ts over tooling/patchnote.json', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'patchnote-test-'));
    const toolingDir = join(tmpDir, 'tooling');
    mkdirSync(toolingDir, { recursive: true });

    const tsConfig = 'export default { prStrategy: { maxStackDepth: 10 } };';
    const jsonConfig: DeepPartial<PatchnoteConfig> = {
      prStrategy: { maxStackDepth: 5 },
    };

    writeFileSync(join(toolingDir, 'patchnote.ts'), tsConfig);
    writeFileSync(join(toolingDir, 'patchnote.json'), JSON.stringify(jsonConfig, null, 2));
  ...
  test('should return defaults on invalid JSON', async () => {
    ...
    // Write invalid JSON
    writeFileSync(join(toolingDir, 'patchnote.json'), '{ invalid json }');
  ...
  test('should return defaults on non-object JSON', async () => {
    ...
    writeFileSync(join(toolingDir, 'patchnote.json'), '["not", "an", "object"]');
  ...
  test('should handle config with all optional fields set', async () => {
    ...
    const fullConfig: DeepPartial<PatchnoteConfig> = {
      expo: {
        enabled: true,
    ...
    writeFileSync(join(toolingDir, 'patchnote.json'), JSON.stringify(fullConfig, null, 2));
  ...
  test('should search parent directories for config file', async () => {
    ...
    mkdirSync(toolingDir, { recursive: true });
    writeFileSync(join(toolingDir, 'patchnote.json'), JSON.stringify({ prStrategy: { maxStackDepth: 7 } }));
