...
export abstract class StateBus extends DispatchQueue<AnyEvent> implements StateBusReader {
  ...
  protected override clearWave(): void {
    this.interestCounts.clear();
    this.interestChanges.clear();
  }

  protected override dispatchWave(eventQueue: readonly (AnyEvent | undefined)[], eventCount: number): void {
    ...
    if (listeners) {
      for (const listener of listeners) {
        try {
          ...
          this.publish({ topic: 'statebus', type: 'error', payload: toError(error) });
        }
      }
    }
  }

  publish(event: AnyEvent): number { return this.queue(event); }
