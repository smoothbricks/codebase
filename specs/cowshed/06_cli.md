...
| `cowshed new <name>`                          | mount path                                       | Clone main → session, minted with the `main` remote (02). `--ref <rev>`, `--from <ws>`, `--browse`, `--slot <n>`, `--register`.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `cowshed mv <ws> <new-name>`                  | new mount path                                   | Rename a workspace, or move the project checkout with `cowshed mv main <new-path>`. Owns the unmount/republish/remount cycle, symmetric with `rm` (02). Refuses a dirty tree, a running job, or an occupied destination, naming the command that clears it.                                                                                                                                                                                                                                                                    |
